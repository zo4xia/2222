...
} catch (err) { console.warn('[empty-catch:src/components/Step1Entry.vue]', err?.message || err) }
...
async function regenerateLayout() {
  ...
  try { localStorage.removeItem('qinghuabu.customTopicLayout') } catch (err) { console.warn('[empty-catch:src/components/Step1Entry.vue]', err?.message || err) }
